...
// Event-Listener hinzufügen
    kursbeschreibung.on('blur', function () {
        ...
        let courseTitle = $('.wisy-kurs-titel').val();
        let courseLearningGoal = $('.wisy-lernziel-text').val();
        let url = '/edit?action=kursniveaustufe';

        if (courseLearningGoal.length > 10) {
    ...
    lernzieltext.on('blur', function () {
        // neuen Text abrufen
        let courseLearningGoal = $(this).val();
        let courseTitle = $('.wisy-kurs-titel').val();
        let url = '/edit?action=kursniveaustufe';
...
//Kompetenzen manuell einfuegen
    $('#stichwortvorschlag').autocomplete({
        ...
        select: function (event, ui) {
            ...
            if (Object.keys(selectedValues).includes(ui.item.value)) {
                return false;
            } else {
                // Hinzufügen des ausgewählten Werts zur Liste der bereits gesuchten Begriffe
                selectedValues[ui.item.value] = 'sachstichwort';
    ...
    $('#kompetenzvorschlag').autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                data: {
                    term: request.term,
                    limit: 10,
                    scheme: "member-skills,skills-hierarchy",
                    onlyrelevant: false
                },
                success: function (data) {
                    let items = [];
                    ...
                    for (var k in data) {
                        /*labels.push({label: data[k].label});
                        uris.push({uri: data[k].uri});*/
                        let label = data[k].label;
                        let uri = data[k].uri;

                        items.push({label: label, value: uri});
                    }
                    response(items);
                }
            });
        ...
        select: function (event, ui) {
            // Überprüfen, ob der ausgewählte Wert bereits gesucht wurde
            if (Object.keys(selectedValues).includes(ui.item.label)) {
                return false;
            } else {
                // Hinzufügen des ausgewählten Werts zur Liste der bereits gesuchten Begriffe
                selectedValues[ui.item.label] = ui.item.value;
                ...
                escoKomp.attr('data-value', ui.item.value);
...
// Füge das closeButton-Element zum escoKomp-Element hinzu
    ...
    $('.wisy-select-hauptkategorie').on('change', function () {
        ...
        $.ajax({
            type: 'POST',
            url: url, // Hier Pfad zur PHP-Datei
