...
function form_control_textarea(
	$name,
	$value,
	$width,
	$height,
	...
	if (isset($addparam['readonly']) && $addparam['readonly']) {
		...
		if (isset($addparam['css_classes']) && $addparam['css_classes']) {
			echo ' class="' . trim($addparam['css_classes']) . '"';
		}
		if (isset($addparam['onchange']) && $addparam['onchange']) {
			echo ' onchange="' . $addparam['onchange'] . '"';
		}
		echo '>' . isohtmlentities(strval($value)) . '</textarea>';
	}
}

function form_control_text(
	$name,
	$value,
	$width = 0,
	$maxlength = 0,
...
Blacklist Handling
 ******************************************************************************/
function blacklist_filter($results, $textonly = false)
{
	$res = array();
	$black = array();
	$db = new DB_Admin();
	$sql = ($textonly)? "SELECT kompetenz_blacklist.title FROM kompetenz_blacklist" : "SELECT kompetenz_blacklist.url FROM kompetenz_blacklist";

	$db->query($sql);
	while ($db->next_record())
		$black[] =  ($textonly) ? utf8_decode($db->Record['title']) : $db->Record['url'];
